...
public class ShallowsSeedLayer extends PrehistoricaGenLayer {
    public ShallowsSeedLayer(TimePeriod period, long seed, GenLayer parent) {
        super(period, seed);
        this.parent = parent;
    }
        ...
        for (int deltaY = 0; deltaY < areaHeight; deltaY++) {
            for (int deltaX = 0; deltaX < areaWidth; deltaX++) {
                ...
                if (isBiomeOceanic(parentBiome)) {
                    ...
                    if (ocean > 0 && ocean < 4) {
                        biomes[index] = Biome.getIdForBiome(this.selectBiome(PrehistoricaBiomeType.OCEAN_SHALLOWS));
                    }
                }
            }
        }
